...
export class QueryExecutor {
  ...
  constructor(
    public queryFunction: (
      query: string
    ) => Promise<SparqlResponse<BlankBinding>>
  ) {}
...
export function processBlankBindings(
  ...
  return loadRelatedBlankNodes(
    relatedBlankBindnings,
    queryExecutor,
    settings
  ).then((loadedGroupsById) => {
    const idsMap = getEncodedIdDictionary(loadedGroupsById);
    const groups = Object.keys(bindingGroupsById).map(
      (key) => bindingGroupsById[key]
    );
...
function getEncodedIdDictionary(
  blankBindingGroups: Dictionary<BlankBinding[]>
): Dictionary<string> {
    ...
    for (const result of results) {
      ...
      if (bindings.length > 0) {
        ...
        recursionPromises.push(
          loadRelatedBlankNodes(
            relatedBlankBindings,
            queryExecutor,
            settings,
            recursionDeep + 1
          ).then((loadedGroupsById) => {
...
function getQueryForChain(
  ...
  sparqlDataProviderSettings: SparqlDataProviderSettings
): string {
...
export function linkTypesOf(params: {
  elementId: string;
}): SparqlResponse<LinkCountBinding> {
  return {
    head: undefined,
...
export function filter(params: FilterParams): SparqlResponse<ElementBinding> {
  ...
  if (params.elementTypeId) {
    ...
    filterResponse.results.bindings = getAllRelatedElements(
      params.refElementId
    );
  }
...
function getAllRelatedByLinkTypeElements(
  ...
  const blankElements = (decodeId(refElementId) || []).concat(
    decodeId(refElementLinkId) || []
  );
      ...
      if (linkDirection === 'in') {
        if (
          ...
          if (isRdfIri(be.blankSrc)) {
            ...
            bindings = bindings.concat(
              decodeId(be.blankSrc.value) || [{ inst: be.blankSrc }]
            );
          }
        } else if (
        ...
        if (
          ...
          if (isRdfIri(be.blankTrg)) {
            ...
            bindings = bindings.concat(
              decodeId(be.blankTrg.value) || [{ inst: be.blankTrg }]
            );
          }
        } else if (
...
function getAllRelatedElements(id: string): ElementBinding[] {
  ...
  if (blankElements) {
    for (const be of blankElements) {
      if (
        be.inst.value === id ||
        id === be.blankSrc.value ||
        id === be.blankTrg.value
      ) {
        bindings.push(be);
        if (isRdfIri(be.blankSrc)) {
