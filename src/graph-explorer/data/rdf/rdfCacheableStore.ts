...
export class RDFCacheableStore {
  ...
  add(rdfGraph: RDFGraph, prefix?: string) {
    this.rdfStorage.add(
      uniqueId(prefix) || uniqueId(DEFAULT_STORAGE_URI),
      rdfGraph
    );
  ...
  matchAll(statements: MatchStatement[]): Promise<RDFGraph> {
    ...
    statements.forEach((statement) => {
      if (
        statement.subject &&
        LABEL_URIS.indexOf(statement.predicate) !== -1 &&
        !statement.object
      ) {
        queries.push(this._getLabels(statement.subject));
      } else if (
        statement.subject &&
        statement.predicate === this.prefs.RDF('type') &&
        !statement.object
      ) {
  ...
  checkElement(id: string): Promise<boolean> {
    if (this.labelsMap[id]) {
      ...
      if (!this.checkingElementMap[id]) {
        this.checkingElementMap[id] = this.rdfStorage
          .match(id, null, null)
          .then((result) => {
  ...
  private enrichMaps(newGraph: RDFGraph): boolean {
    ...
    // Deal with counts
    const typeInstances = newGraph
      .match(null, this.prefs.RDF('type'), null)
      .toArray();
    ...
    Object.keys(typeInstMap).forEach(
      (key) => (this.countMap[key] = typeInstMap[key].length)
    );
  ...
  private multipleMatch(statements: MatchStatement[]): RDFGraph {
    const triples: Triple[] = [];
    const graphs = Object.keys(this.rdfStorage.graphs).map(
      (id) => this.rdfStorage.graphs[id]
    );
    for (const graph of graphs) {
      for (const triple of graph._graph) {
        for (const statement of statements) {
          if (
            (!statement.object ||
              statement.object === triple.object.nominalValue) &&
            (!statement.predicate ||
              statement.predicate === triple.predicate.nominalValue) &&
            (!statement.subject ||
              statement.subject === triple.subject.nominalValue)
          ) {
            triples.push(triple);
            continue;
          }
