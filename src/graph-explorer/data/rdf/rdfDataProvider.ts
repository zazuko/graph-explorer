...
export class RDFDataProvider implements DataProvider {
  ...
  async classTree(): Promise<ClassModel[]> {
    ...
    for (const triple of subClasses) {
      ...
      if (
        isNamedNode(triple.object) &&
        parents[subClassIRI].indexOf(classIRI) === -1
      ) {
  ...
  async linkTypes(): Promise<LinkType[]> {
    ...
    const [rdfLinks, owlLinks] = await Promise.all([
      rdfLinksQueries,
      owlLinksQueries,
    ]);
  ...
  async linksInfo(params: {
    ...
    const graph = await this.rdfStorage.matchAll(
      statements.filter((statement) => statement)
    );
    let triples;
  ...
  async linkTypesOf(params: { elementId: ElementIri }): Promise<LinkCount[]> {
    ...
    const incomingElementsTriples = await this.rdfStorage.match(
      null,
      null,
      element
    );
    ...
    const outgoingElementsTriples = await this.rdfStorage.match(
      element,
      null,
      null
    );
  ...
  async filter(params: FilterParams): Promise<Dictionary<ElementModel>> {
    ...
    if (params.elementTypeId) {
      elementsPromise = this.filterByTypeId(
      ...
      const filteredElements = await this.getAllElements(
        params.text,
        limitCallback
      );
  ...
  private async filterByTypeId(
    ...
    return Promise.all(
      elementTriples
        ...
        .map((el) =>
          this.getElementInfo(el.subject.nominalValue as ElementIri, true)
        )
    );
  }
    ...
    if (linkDirection === 'in') {
      ...
      return Promise.all(
        elementTriples
          ...
          .map((el) =>
            this.getElementInfo(el.object.nominalValue as ElementIri, true)
          )
      );
    }
  }
  ...
  private async getAllElements(
    ...
    for (const triple of elementTriples.toArray()) {
      if (filter(triple.subject, promices.length)) {
        ...
        if (containsKey) {
          promices.push(
            this.getElementInfo(triple.subject.nominalValue as ElementIri, true)
          );
        }
      }
    }
    ...
    if (key) {
      for (const el of elements) {
        ...
        for (const label of el.label.values) {
          acceptableKey =
            acceptableKey || label.value.toLowerCase().indexOf(key) !== -1;
          if (acceptableKey) {
            break;
          }
        }
        acceptableKey =
          acceptableKey || el.id.toLowerCase().indexOf(key) !== -1;
        if (acceptableKey) {
          result[el.id] = el;
        }
      }
  ...
  private getTypes(el: ElementIri): Promise<ElementTypeIri[]> {
    return this.rdfStorage
      .match(el, PREFIX_FACTORIES.RDF('type'), null)
      .then((typeTriples) => {
        return typeTriples
          .toArray()
          .map((t) => t.object.nominalValue as ElementTypeIri);
      });
  }
