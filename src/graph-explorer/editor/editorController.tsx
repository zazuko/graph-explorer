...
export class EditorController {
      ...
      halo = (
        <Halo
          ...
          onToggleNavigationMenu={() => {
            if (
              this.dialogTarget &&
              this.dialogType === DialogTypes.ConnectionsMenu
            ) {
              this.hideDialog();
            } else {
          ...
          onFollowLink={(element, e) =>
            this.view.onIriClick(
              element.iri,
              element,
              IriClickIntent.JumpToEntity,
              e
            )
          }
        />
  ...
  showEditEntityForm(target: Element) {
    ...
    const isIriModified = AuthoringState.isElementWithModifiedIri(
      this.authoringState,
      target.data.id
    );
    let modelToEdit;
      ...
      <EditElementTypeForm
        ...
        onChangeElement={(data: ElementModel) => {
          ...
          this.setTemporaryState(
            TemporaryState.deleteElement(this.temporaryState, previous)
          );
          target.setData(data);
          this.setTemporaryState(
            TemporaryState.addElement(this.temporaryState, data)
          );
        }}
  ...
  changeEntityData(targetIri: ElementIri, newData: ElementModel) {
    ...
    const event =
      newState.elements.get(targetIri) || newState.elements.get(newData.id);
    this.model.history.execute(
      setElementData(this.model, targetIri, event.after)
    );
    this.setAuthoringState(newState);
  ...
  createNewLink(params: { link: Link; temporary?: boolean }): Link {
    ...
    const links = this.model.links.filter((link) =>
      sameLink(link.data, base.data)
    );
    if (links.length > 0) {
      if (temporary) {
        this.setTemporaryState(
...
class LoadingWidget extends React.Component<LoadingWidgetProps, {}> {
  ...
  render() {
    ...
    return (
      <div className="graph-explorer-loading-widget">
...
function makeLinkWithDirection(original: Link, data: LinkModel): Link {
  ...
  if (
    !(
      data.targetId === original.data.sourceId ||
