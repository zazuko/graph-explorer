...
export class ConnectionsMenu extends React.Component<ConnectionsMenuProps, {}> {
    ...
    editor.model.dataProvider
      ...
      .then((linkTypes) => {
        ...
        this.context.workspace.triggerWorkspaceEvent(
          WorkspaceEventKey.connectionsLoadLinks
        );
      })
      .catch((err) => {
  ...
  private addSelectedElements = (selectedObjects: ReactElementModel[]) => {
    ...
    const hasChosenLinkType =
      this.linkDataChunk && linkType !== ALL_RELATED_ELEMENTS_LINK;
...
class ConnectionsMenuMarkup extends React.Component<
  ConnectionsMenuMarkupProps,
  ConnectionsMenuMarkupState
> {
  ...
  private getBreadCrumbs = () => {
    if (this.props.objectsData && this.state.panel === 'objects') {
      ...
      return (
        <span className="graph-explorer-connections-menu_bread-crumbs">
          <a
  ...
  private getBody = () => {
    if (this.props.state === 'error') {
      ...
      if (this.props.state === ProgressState.loading) {
        return (
          <label className="graph-explorer-label graph-explorer-connections-menu__loading">
            Loading...
          </label>
        );
      }
  ...
  render() {
    return (
      <div className="graph-explorer-connections-menu">
        <label className="graph-explorer-label graph-explorer-connections-menu__title-label">
          {this.getTitle()}
        </label>
        {this.getBreadCrumbs()}
        <div className="graph-explorer-connections-menu_search-line">
          <input
            type="text"
...
class ConnectionsList extends React.Component<
  ...
  { scores: Dictionary<PropertyScore> }
> {
  ...
  private getProbableLinks = () => {
    ...
    return (this.props.data.links || [])
      .filter((link) => {
        return (
          this.state.scores[link.id] &&
          (this.state.scores[link.id].score > 0 || isSmartMode)
        );
      })
        ...
        <LinkInPopupMenu
          ...
          probability={
            this.state.scores[link.id] && notSure
              ? this.state.scores[link.id].score
              : 0
          }
        />
      );
  ...
  render() {
    ...
    const probableLinks = this.getProbableLinks().filter(
    ...
    if (views.length === 0 && probableViews.length === 0) {
      viewList = (
        <label className="graph-explorer-label graph-explorer-connections-menu_links-list__empty">
          List empty
        </label>
      );
    } else {
...
> {
  ...
  private onSelectAll = () => {
    ...
    const allSelected = allNonPresentedAreSelected(
      objects,
  ...
  private getFilteredObjects(): ReactElementModel[] {
    ...
    return this.props.data.objects.filter((element) => {
      const text = this.props.view
        .formatLabel(element.model.label.values, element.model.id)
        .toLowerCase();
  ...
  private counter = (activeObjCount: number) => {
    ...
    const wrongNodes =
      Math.min(MAX_LINK_COUNT, this.props.data.linkDataChunk.expectedCount) -
      this.props.data.objects.length;
    const wrongNodesString =
      Math.abs(wrongNodes) > MAX_LINK_COUNT
        ? `${MAX_LINK_COUNT}+`
  ...
  render() {
    ...
    return (
      <div className="graph-explorer-connections-menu_objects-panel">
        <div className="graph-explorer-connections-menu_objects-panel__select-all">
          <label>
            <input
              type="checkbox"
        ...
        {this.props.loading ? (
          <label className="graph-explorer-label graph-explorer-connections-menu__loading-objects">
            Loading...
          </label>
        ) : objects.length === 0 ? (
          <label className="graph-explorer-label graph-explorer-connections-menu__loading-objects">
          ...
          <div className="graph-explorer-connections-menu_objects-panel_objects-list">
            <SearchResults
              view={this.props.view}
            ...
            {this.props.data.linkDataChunk.expectedCount > MAX_LINK_COUNT ? (
              <div
                ...
                onClick={() =>
                  this.props.onMoveToFilter(this.props.data.linkDataChunk)
                }
              >
                The list was truncated, for more data click here to use the
                filter panel
              </div>
            ) : null}
          </div>
        )}
        <div className="graph-explorer-connections-menu_objects-panel_bottom-panel">
