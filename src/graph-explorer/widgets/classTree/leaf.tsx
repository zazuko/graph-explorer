...
export class Leaf extends React.Component<LeafProps, State> {
  ...
  componentDidUpdate(nextProps: LeafProps) {
      ...
      <div className={LEAF_CLASS} role="tree-item">
        <div className={`${LEAF_CLASS}__row`}>
          ...
          <a className={bodyClass} href={node.model.id} onClick={this.onClick}>
            ...
            {node.model.count ? (
              <span className={`${LEAF_CLASS}__count graph-explorer-badge`}>
                {node.model.count}
              </span>
            ) : null}
          </a>
          {creatableClasses.get(node.model.id) ? (
            <div
              className={`${LEAF_CLASS}__create graph-explorer-btn-group graph-explorer-btn-group-xs`}
            >
              <button
