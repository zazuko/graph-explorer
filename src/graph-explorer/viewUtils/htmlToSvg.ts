...
export function htmlToSvg(
  ...
  function processText(textElement: Text): SVGGElement[] {
    ...
    if (
      ...
      !(textElement.parentNode instanceof Element)
    ) {
      return [];
    }
    ...
    lines.forEach((line, index) => {
      ...
      text.setAttribute(
        'transform',
        `translate(${x}, ${y + index * textSize.height})`
      );
...
function getBorderRadius(style: CSSStyleDeclaration): number[] {
  const radius =
    Math.round(stringToNumber(style.borderRadius.split(' ')[0])) || 0;
  return [
...
function getOffsetTop(element: HTMLElement): number {
  if (element.className !== 'graph-explorer-overlayed-element') {
    return (
      element.offsetTop +
      element.clientTop +
      (element.offsetParent && element.offsetParent instanceof HTMLElement
        ? getOffsetTop(element.offsetParent)
        : 0)
    );
  } else {
    return 0;
...
function getOffsetLeft(element: HTMLElement): number {
  if (element.className !== 'graph-explorer-overlayed-element') {
    return (
      ...
      element.clientLeft +
      (element.offsetParent && element.offsetParent instanceof HTMLElement
        ? getOffsetLeft(element.offsetParent)
        : 0)
    );
  } else {
    return 0;
...
function getTextSize(text: string, font: string) {
  ...
  context.font = font;
  const fontSizeString = context.font
    .split(' ')
    .filter((term) => term.indexOf('px') !== -1)[0];
...
function cutExtension(value: string): string {
  ...
  if (
    postfixes.filter((postfix) => lowerCaseValue.indexOf(postfix) !== -1)
      .length > 0
  ) {
