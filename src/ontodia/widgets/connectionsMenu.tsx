  ...
  private loadLinks() {
    ...
    editor.model.dataProvider
      ...
      .then((linkTypes) => {
        ...
        this.context.ontodiaWorkspace.triggerWorkspaceEvent(
...
> {
    ...
    if (this.props.objectsData && this.state.panel === 'objects') {
      ...
      return (
        <span className="ontodia-connections-menu_bread-crumbs">
          <a
            className="ontodia-connections-menu__link"
  ...
  private getBody = () => {
    if (this.props.state === 'error') {
      return (
        <label className="ontodia-label ontodia-connections-menu__error">
          Error
        </label>
      );
      ...
      if (this.props.state === ProgressState.loading) {
        return (
          <label className="ontodia-label ontodia-connections-menu__loading">
            Loading...
          </label>
        );
      }
  ...
  private renderSortSwitch = (id: string, icon: string, title: string) => {
    return (
      <div>
        ...
        <label
          htmlFor={id}
          className="ontodia-connections-menu__sort-switch-label"
          title={title}
        >
          <i className={`fa ${icon}`} />
        </label>
  ...
  render() {
    return (
      <div className="ontodia-connections-menu">
        <label className="ontodia-label ontodia-connections-menu__title-label">
          {this.getTitle()}
        </label>
        {this.getBreadCrumbs()}
...
> {
  ...
  render() {
    ...
    if (views.length === 0 && probableViews.length === 0) {
      viewList = (
        <label className="ontodia-label ontodia-connections-menu_links-list__empty">
          List empty
        </label>
      );
    } else {
      ...
      if (views.length > 1 || (isSmartMode && probableViews.length > 1)) {
        ...
        viewList = [
          ...
          <hr
            key="ontodia-hr-line"
            className="ontodia-connections-menu_links-list__hr"
          />,
        ].concat(viewList);
      }
    }
  ...
  private counter = (activeObjCount: number) => {
    ...
    return (
      <div className="ontodia-label ontodia-connections-menu_objects-panel_bottom-panel__count-label">
        ...
        <span
          className="ontodia-connections-menu_objects-panel_bottom-panel__extra-elements"
  ...
  render() {
    ...
    return (
      <div className="ontodia-connections-menu_objects-panel">
        ...
        {this.props.loading ? (
          <label className="ontodia-label ontodia-connections-menu__loading-objects">
            Loading...
          </label>
        ) : objects.length === 0 ? (
          <label className="ontodia-label ontodia-connections-menu__loading-objects">
