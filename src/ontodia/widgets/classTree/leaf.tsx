...
const EXPAND_ICON = require<
  string
>('../../../../images/tree/expand-toggle.svg');
const COLLAPSE_ICON = require<
  string
>('../../../../images/tree/collapse-toggle.svg');
const DEFAULT_LEAF_ICON = require<
  string
>('../../../../images/tree/leaf-default.svg');
const DEFAULT_PARENT_ICON = require<
  string
>('../../../../images/tree/leaf-folder.svg');
...
export class Leaf extends React.Component<LeafProps, State> {
  ...
  render() {
    ...
    if (!icon) {
      icon =
        node.derived.length === 0 ? DEFAULT_LEAF_ICON : DEFAULT_PARENT_ICON;
    }
    ...
    const label = highlightSubstring(node.label, searchText, {
      className: `${LEAF_CLASS}__highlighted-term`,
    });
    ...
    return (
      <div className={LEAF_CLASS} role="tree-item">
        <div className={`${LEAF_CLASS}__row`}>
          ...
          >
            {toggleIcon ? (
              <img className={`${LEAF_CLASS}__toggle-icon`} src={toggleIcon} />
            ) : null}
          </div>
          ...
          {creatableClasses.get(node.model.id) ? (
            <div
              className={`${LEAF_CLASS}__create ontodia-btn-group ontodia-btn-group-xs`}
            >
              <button
